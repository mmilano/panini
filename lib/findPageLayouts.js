...
module.exports = function(pageLayouts) {
  ...
  for (var dir in pageLayouts) {
    ...
    if (this.debug) {
        console.log ("examining:  " + dir + ":" + template);
    }

    // break down the path of the directory
    var pathBreakdown = parseGlob(dir);
    var isAGlob = pathBreakdown.is.glob;
    ...
    if (this.debug) {
        console.log("-------");
        console.log("constructed pagelayouts table: ");
        
        for (var i in pageLayoutMap) {
            console.log ("* " + i + ":" + pageLayoutMap[i]);
        }
        console.log("\n");
    }
